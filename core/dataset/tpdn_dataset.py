...
class TPDNDataset(Dataset):
  ...
  def __init__(self, datafile):
    ...
    self.data["fillers"] = self.data["fillers"].map(lambda a: np.fromstring(a, sep=" ", dtype=int))
    self.data["roles"] = self.data["roles"].map(lambda a: np.fromstring(a, sep=" ", dtype=int))
    self.data["annotation"] = self.data["annotation"].map(lambda a: np.fromstring(a, sep=" ", dtype=int))
    self.data["encoding"] = self.data["encoding"].map(lambda a: np.fromstring(a, sep=" ", dtype=np.float32))
    self.data["target"] = self.data["target"].map(lambda a: np.fromstring(a, sep=" ", dtype=int))
