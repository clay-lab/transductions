...
class TransductionDataset:
    ...
    def transform_field(self):
        if self._trns_field == "source":
            ...
            log.error(
                "`transform_field` must be either 'source' or 'target'; you supplied {}!".format(
                    self.trns_field
                )
            )
    ...
    def _create_regular_splits(self, cfg: DictConfig):
        ...
        with open(self._raw_path) as raw_data:
            ...
            for line in raw_data:
                if "withholding" in cfg.dataset.keys() and bool(
                    to_withhold.search(line)
                ):
    ...
    def __init__(self, cfg: DictConfig, device, fields: Dict = None, BERT=False):
        ...
        partial_path = (
            hydra.utils.get_original_cwd() if HydraConfig.initialized() else os.getcwd()
        )
        ...
        if fields is not None:
            ...
            if source_format == "sequence" and target_format == "sequence":
                ...
                self.target_field = Field(
                    lower=True, eos_token="<eos>", init_token="<sos>"
                )
            else:
                raise NotImplementedError
        ...
        for attr in self.__dict__:
            ...
            if "_iterators" in attr:
                ...
                for i, s in enumerate(getattr(self, attr)):
                    if i > 0:
                        message += ", "
                    message += "{} ({} sequences)".format(
                        s, len(getattr(self, attr)[s].dataset) + 1
                    )
                message += "]\n"
